"""HotpotQA evaluation with LangGraph + Orla: hybrid on-prem + cloud routing.

Identical multi-stage decomposition pipeline to ``hotpotqa_routing`` but with
a three-tier hybrid backend configuration:

- **Cheap** (on-prem): Qwen3 4B Instruct on a local GPU via vLLM.
- **Mid** (on-prem): Qwen3 32B on a local GPU via vLLM.
- **Strong** (cloud): Qwen3 235B A22B on Bedrock Mantle.

Easy questions stay entirely on-prem (~1.3s), medium questions use the larger
local GPU (~1.3s), and only hard questions incur the cloud round-trip (~3.6s).
This exploits the real latency gap between local inference and a managed API
while preserving frontier accuracy on difficult questions.

Three modes compare cost, accuracy, and latency:

- **baseline**: all stages route to the strong cloud model.
- **all-cheap**: all stages route to the cheap on-prem model.
...
- Two vLLM instances serving the on-prem models on separate GPUs::
  ...
  Or use ``docker compose -f deploy/docker-compose.hotpotqa-vllm.yaml up -d``.

- ``OPENAI_API_KEY`` set to a valid Bedrock Mantle key (vLLM ignores it).
...
Run from the ``pyorla`` directory::
    ...
    uv run python examples/hotpotqa_routing_hybrid/run.py --mode routed --limit 20
    uv run python examples/hotpotqa_routing_hybrid/run.py --mode baseline --limit 20
    uv run python examples/hotpotqa_routing_hybrid/run.py --mode all-cheap --limit 20
"""
...
DEFAULT_MID_ENDPOINT = "http://localhost:8000/v1"
DEFAULT_STRONG_ENDPOINT = MANTLE_BASE_URL

DEFAULT_CHEAP_MODEL = "openai:Qwen/Qwen3-4B-Instruct-2507"
DEFAULT_MID_MODEL = "openai:Qwen/Qwen3-32B"
...
def _register_backends(client: OrlaClient) -> LLMBackend:
    """Register cheap / mid / strong backends for hybrid on-prem + cloud.
    ...
    - Cheap and mid backends point at local vLLM instances (on-prem GPUs).
    - Strong backend points at Bedrock Mantle (cloud).

    Cost models reflect both token pricing and the latency premium of cloud
    calls so Orla's cost-aware routing prefers on-prem when quality allows.
    """
    api_key_env = "OPENAI_API_KEY"
    cheap = LLMBackend(
        name="hotpot-cheap",
        endpoint=_env("HOTPOTQA_CHEAP_ENDPOINT", DEFAULT_CHEAP_ENDPOINT),
        type="openai",
    ...
    mid = LLMBackend(
        name="hotpot-mid",
        endpoint=_env("HOTPOTQA_MID_ENDPOINT", DEFAULT_MID_ENDPOINT),
        type="openai",
    ...
    strong = LLMBackend(
        name="hotpot-strong",
        endpoint=_env("HOTPOTQA_STRONG_ENDPOINT", DEFAULT_STRONG_ENDPOINT),
        type="openai",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "HotpotQA benchmark with LangGraph + Orla: hybrid on-prem vLLM "
            "+ Bedrock cloud routing (multi-stage decomposition pipeline)."
        ),
    )
    parser.add_argument(
        "--mode",
        ...
        default="routed",
        help=(
            "baseline: always strong cloud model. "
            "all-cheap: always cheap on-prem model. "
    ...
    if not _env("OPENAI_API_KEY", ""):
        raise SystemExit(
            "OPENAI_API_KEY must be set for the Bedrock Mantle strong backend."
        )
